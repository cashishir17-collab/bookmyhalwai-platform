...
export default function AnalyticsPage() {
  ...
  useEffect(() => { const load = async () => { const firestore = db; if (!firestore) return; const names = ["vendors","enquiries","bookings","reviews"] as const; const snapshots = await Promise.all(names.map((name) => getDocs(collection(firestore, name)))); setData(Object.fromEntries(names.map((name,index) => [name, snapshots[index].docs.map((item) => item.data())])) as typeof data); }; void load(); }, []);
